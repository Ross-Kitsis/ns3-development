...
#ifndef SRC_THESISINTERNETROUTING_MODEL_GEOREQUESTHEADER_H_
#define SRC_THESISINTERNETROUTING_MODEL_GEOREQUESTHEADER_H_
...
class GeoRequestHeader : public Header
{
public:
	GeoRequestHeader(Vector m_OriginPosition = Vector(), Vector m_OriginVelocity = Vector(), Time m_timestamp = Time(),
								 bool m_isDtnTolerant = false, Vector m_SenderPosition = Vector(), Vector m_SenderVelocity = Vector(),
								 Ipv6Address m_RsuAddress = Ipv6Address(), uint8_t m_hopCount = 0, Vector m_QueryPosition = Vector());

	virtual ~GeoRequestHeader();
	...
	void SetQueryPosition(Vector p){m_QueryPosition = p;}
	Vector GetQueryPosition() {return m_QueryPosition;}

	bool operator== (GeoRequestHeader const & o) const;
...
private:
	...
	/**
	 * Query Position (Where the geoquery is being routed towards)
	 */
...
#endif /* SRC_THESISINTERNETROUTING_MODEL_GEOREQUESTHEADER_H_ */
