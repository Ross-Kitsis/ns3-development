                              ...
                              Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
  ...
  //////////// Insert transmission book keeping

	Transmission t;
	t.Destination = header.GetDestination();
            ...
            if (lcb.IsNull () == false)
              {
                ...
                NS_LOG_LOGIC ("Broadcast properties: " );
                NS_LOG_LOGIC ("IIF properties: " << iif  << " NetDevice:" << m_ipv4 -> GetNetDevice(iif)->GetTypeId().GetName());
                lcb (p, header, iif);
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  {
  	NS_LOG_LOGIC("NotifyInterfaceUp returned address on interface" << i << " with address: " << iface.GetLocal() );
    return;
  }
  ...
  m_socketAddresses.insert (std::make_pair (socket, iface));
	...
	NS_LOG_LOGIC("Bound socket to " << Ipv4Address::GetAny ());
  ...
  m_socketSubnetBroadcastAddresses.insert (std::make_pair (socket, iface));
	...
	NS_LOG_LOGIC("Bound socket to " << iface.GetBroadcast ());
      ...
      if (!socket)
        {
          if (iface.GetLocal () == Ipv4Address ("127.0.0.1"))
          {
          	NS_LOG_LOGIC ("Local address returned: " << iface.GetLocal () );
            return;
          }
          // Create a socket to listen only on this interface
          Ptr<Socket> socket = Socket::CreateSocket (GetObject<Node> (),
          ...
          m_routingTable.AddRoute (rt);
          NS_LOG_LOGIC (this << "Bound socket to: " << iface.GetLocal () << " and " << iface.GetBroadcast());
        }
    }
  else
    {
...
int32_t
RoutingProtocol::GetP2pInterface()
{
	...
	{
		if(m_ipv4->GetNetDevice(j)->GetInstanceTypeId().GetName().compare(p2pd.GetTypeId().GetName()) == 0)
		{
			interface = j;
			break;
		}
...
int32_t
RoutingProtocol::GetWirelessInterface()
{
	uint32_t interface = -1;
	WifiNetDevice wifi;
	...
	for(uint32_t j = 0; j < m_ipv4 -> GetNInterfaces(); j++)
	{
		if(m_ipv4->GetNetDevice(j)->GetInstanceTypeId().GetName().compare(wifi.GetTypeId().GetName()) == 0)
		{
			interface = j;
			break;
		}
