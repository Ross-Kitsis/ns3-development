...
NS_LOG_COMPONENT_DEFINE ("McastNeighbors");
...
void
Neighbors::Update (Ipv6Address addr, Time expire, Vector velocity, Vector position)
{
  ...
  Neighbor neighbor (addr, LookupMacAddress (addr), expire + Simulator::Now (), velocity, position);
...
Neighbors::LookupMacAddress (Ipv6Address addr)
{
  ...
  for (std::vector<Ptr<NdiscCache> >::const_iterator i = m_arp.begin ();
       i != m_arp.end (); ++i)
    {
      ...
      if (entry != 0 && (entry->IsReachable()) && !entry->IsStale())
        {
